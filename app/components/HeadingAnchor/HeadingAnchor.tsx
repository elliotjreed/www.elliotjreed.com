...
export const HeadingAnchor = ({ id, level = 2, children }: HeadingAnchorProps): ReactElement => {
  ...
  const linkButton: ReactElement = (
    <button
      ...
      className="ml-2 inline-flex cursor-pointer items-center opacity-20 transition-opacity group-hover:opacity-100 focus:opacity-100"
    ...
    >
      ...
      {copied && <span className="ml-1 text-xs whitespace-nowrap text-gray-600 dark:text-gray-400">Copied!</span>}
    </button>
  );
